...
@Suite struct AgentRisk {
    ...
    @Test func shellCommands() {
        ...
        #expect(ShellSafety.risk("echo hi > /etc/hosts") == .destructive)
        #expect(ShellSafety.risk("echo hi > /tmp/bolo-surely-not-here-\(UUID().uuidString).txt") == .write)
        #expect(ShellSafety.risk("mkdir -p /tmp/x && cat > /tmp/x/New-\(UUID().uuidString).java << 'EOF'\nclass A {}\nEOF") == .system)
